...
class AutobahnRemoteError extends Error {
    constructor(msg, remote) {
        super(msg);
        this.name = "AutobahnRemoteError";
        if (remote) {
            this.remote = {
                args : remote.args,
                name : remote.name,
...
function wrapPromise(p) {
    let obj = {};
    Error.captureStackTrace(obj, wrapPromise);
    return p.catch(err => {
        err.stack = obj.stack;
...
var Session = function (socket, defer, onchallenge, on_user_error, on_internal_error) {
    ...
    self._subscriptions = {};
    self.transformObject = function(obj) {
        return obj;
    };
    self.preError = function (x) {
        return x;
    };
    self.transformError = function (err) {
        var name = "";
        var args = [];
        var kwargs = {};
        name = 'wamp.error.runtime_error';
        args = [err];
        return {
            name,
            args,
    ...
    self._protocol_violation = function (reason) {
        ...
        util.handle_error(self._on_internal_error, new Error("failing transport due to protocol violation: " + reason))
    };
    ...
    self._process_SUBSCRIBE_ERROR = function (msg) {
        ...
        if (request in self._subscribe_reqs) {
            ...
            var details = msg[3];
            var error = self.preError(new AutobahnRemoteError("Error while subscribing.", {
                name : msg[4],
                args : msg[5],
                kwargs : msg[6]
            }));
    ...
    self._process_UNSUBSCRIBE_ERROR = function (msg) {
        ...
        if (request in self._unsubscribe_reqs) {
            ...
            var details = msg[3];
            var error = self.preError(new AutobahnRemoteError("Error while unsubscribing.", {
                name : msg[4],
                args : msg[5],
                kwargs : msg[6]
            }));
    ...
    self._process_PUBLISH_ERROR = function (msg) {
        ...
        if (request in self._publish_reqs) {
            ...
            var details = msg[3];
            var error = self.preError(new AutobahnRemoteError("Error while publishing.", {
                name : msg[4],
                args : msg[5],
                kwargs : msg[6]
            }));
    ...
    self._process_REGISTER_ERROR = function (msg) {
        ...
        if (request in self._register_reqs) {
            var details = msg[3];
            var error = self.preError(new AutobahnRemoteError("Error while registering operation.", {
                name : msg[4],
                args : msg[5],
                kwargs : msg[6]
            }));
    ...
    self._process_UNREGISTER_ERROR = function (msg) {
        ...
        if (request in self._unregister_reqs) {
            ...
            var details = msg[3];
            var error = self.preError(new AutobahnRemoteError("Error while unregistering.", {
                name : msg[4],
                args : msg[5],
                kwargs : msg[6]
            }));
    ...
    self._process_CALL_ERROR = function (msg) {
        ...
        if (request in self._call_reqs) {
            ...
            var details = msg[3];
            var error = self.preError(new AutobahnRemoteError("Error during RPC call.", {
                name : msg[4],
                args : msg[5],
                kwargs : msg[6]
            }));
        ...
        if (registration in self._registrations) {
            ...
            when_fn.call(reg.endpoint, args, kwargs, cd).then(
                ...
                function (err) {
                    ...
                    var res = self.transformError(err);
                    var reply = [MSG_TYPE.ERROR, MSG_TYPE.INVOCATION, request, {}, res.name, res.args, res.kwargs];
                    self._send_wamp(reply);
        ...
        if (!self._id) {
            ...
            if (msg_type === MSG_TYPE.WELCOME) {
                ...
                if (self._onchallenge) {
                    ...
                    util.handle_error(self._on_internal_error, new Error("received WAMP challenge, but no onchallenge() handler set"));
...
Session.prototype.call = function (procedure, args, kwargs, options) {
    ...
    kwargs = this.transformObject(kwargs);
    args = args && args.map(x => this.transformObject(x));
    var self = this;
